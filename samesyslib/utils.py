...
def dataset_summary(data, index_summary=True, detail=False, quick=True):
    ...
    if not quick:
        df_s = df_s.merge(
            pd.DataFrame(df.mean(numeric_only=None), columns=["mean"]),
            left_index=True,
            right_index=True,
            how="outer",
        )
        df_s = df_s.merge(
            pd.DataFrame(df.std(), columns=["std"]),
            left_index=True,
            right_index=True,
            how="outer",
        )
    df_s = df_s.merge(
        pd.DataFrame(df.nunique(), columns=["n_uniq"]),
        left_index=True,
        right_index=True,
        how="outer",
    )
    df_s = df_s.merge(
        pd.DataFrame(df.isnull().sum(), columns=["n_miss"]),
        left_index=True,
        right_index=True,
        how="outer",
    )
    df_s = df_s.merge(
        pd.DataFrame(df.count(), columns=["obs"]),
        left_index=True,
        right_index=True,
        how="outer",
    )
...
def init_metadata(
    conn: object, table_name: str = None, schema: str = None, metadata: dict = None
) -> None:
    """
...
def split_array_into_batches(shop_list: object, batch_size: int):
    if shop_list.shape[0] > batch_size:
        return np.array_split(
            shop_list.values, np.ceil(shop_list.shape[0] / batch_size)
        )
    else:
...
def preprocess_activities(
    ...
    activities_column: str = "occasion_type_id",
    prefix: str = "event_",
):
    """
    ...
    """
    activities = pd.get_dummies(
        shop_data.set_index("date")[activities_column]
        .apply(lambda x: json.loads(x) if pd.notnull(x) else None)
        .explode()
    )
    # rename columns
    ...
    if activities.columns.size > 0:
        ...
        for col in activities.columns:
            ...
            cum_val = cum_val.join(
                cum_val.groupby("grouper")[col].agg(max_value=("max")),
                on=["grouper"],
                how="left",
            ).drop("grouper", axis=1)
...
def remove_special_char(
    my_str: str, replace_ws: bool = False, escape_punct: bool = True
) -> str:
...
def filter_rows_and_cols(df: pd.DataFrame, filter_list: list):
    ...
    return df.loc[df.index.isin(filter_list), df.columns.astype(int).isin(filter_list)]
...
def get_branch_commit_from_mapping(query, conn):
    ...
    ), "No git branch and commit info saved in metadata mapping"
